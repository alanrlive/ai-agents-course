...
def run_pipeline(image_path: str) -> None:
    ...
    with ctx as trace:
        ...
        vision_output = run_vision_agent(image_path, logger=logger)
        ...
        if vision_output.rejected:
            ...
            logger.pipeline_error(error=vision_output.rejection_reason, stage="vision_agent")
            if trace:
        ...
        parser_message = run_note_parser(vision_output.extracted_text, logger=logger)
        ...
        if parser_message["status"] == STATUS_ERROR:
            ...
            logger.pipeline_error(
                error=parser_message["payload"].get("parse_error", "unknown"),
                stage="note_parser",
            )
            if trace:
        ...
        if planner_message is None:
            logger.pipeline_error(error="action_planner produced no output", stage="action_planner")
            return
        ...
        # Determine outcome and close the structured log
        if planner_message["status"] == STATUS_ERROR:
            logger.pipeline_error(
                error=planner_message["payload"].get("parse_error", "action_planner_failed"),
                stage="action_planner",
            )
        elif critic_message is not None and critic_message["status"] == STATUS_ERROR:
            logger.pipeline_error(
                error=critic_message["payload"].get("parse_error", "critic_failed"),
                stage="critic",
            )
        elif critic_message is not None and critic_message["status"] == STATUS_APPROVED:
            logger.pipeline_complete(outcome="approved")
        else:
            logger.pipeline_complete(outcome="max_revisions_reached")

        print(f"  Trace ID       : {logger.trace_id}")
        print(f"  Structured log : {logger._log_path}")

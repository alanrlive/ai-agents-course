...
def run_note_parser(raw_notes: str, logger: PipelineLogger | None = None) -> dict:
    if logger:
        logger.agent_start(NOTE_PARSER)
    ...
    text, input_tokens, output_tokens = call_llm(NOTE_PARSER_PROMPT, user_message, NOTE_PARSER)
    ...
    if status == STATUS_SUCCESS:
        ...
        if logger:
            logger.agent_complete(NOTE_PARSER, input_tokens=input_tokens, output_tokens=output_tokens)
    else:
        print("[Note Parser] ERROR — failed to parse JSON response")
        if logger:
            logger.agent_error(NOTE_PARSER, error=payload.get("parse_error", "JSON parse error"))
...
def run_action_planner(
    ...
    if logger:
        logger.agent_start(ACTION_PLANNER, revision_round=revision_round)
    ...
    text, input_tokens, output_tokens = call_llm(ACTION_PLANNER_PROMPT, user_message, ACTION_PLANNER, revision_round)
    ...
    if status == STATUS_SUCCESS:
        ...
        if logger:
            logger.agent_complete(ACTION_PLANNER, input_tokens=input_tokens, output_tokens=output_tokens, revision_round=revision_round)
    else:
        print(f"[Action Planner] ERROR — failed to parse JSON response (round {revision_round})")
        if logger:
            logger.agent_error(ACTION_PLANNER, error=payload.get("parse_error", "JSON parse error"), revision_round=revision_round)
...
def run_critic(planner_message: dict, parser_message: dict, logger: PipelineLogger | None = None) -> dict:
    extracted = json.dumps(parser_message["payload"], indent=2)
    register = json.dumps(planner_message["payload"], indent=2)
    revision_round = planner_message["revision_round"]

    if logger:
        logger.agent_start(CRITIC, revision_round=revision_round)
    ...
    text, input_tokens, output_tokens = call_llm(CRITIC_PROMPT, user_message, CRITIC, revision_round)
    ...
    if status in (STATUS_APPROVED, STATUS_REVISION_REQUIRED):
        ...
        if logger:
            logger.agent_complete(CRITIC, input_tokens=input_tokens, output_tokens=output_tokens, revision_round=revision_round)
    else:
        print(f"[Critic] ERROR — failed to parse JSON response (round {revision_round})")
        if logger:
            logger.agent_error(CRITIC, error=payload.get("parse_error", "JSON parse error"), revision_round=revision_round)

...
def run_pipeline(raw_notes: str) -> None:
    # ----------------------------------------------------------------
    # Step 1 — Note Parser
    # ----------------------------------------------------------------
    print(f"\n{BAR}")
    ...
    parser_message = run_note_parser(raw_notes)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--notes",
        default="sample_notes.txt",
        help="Path to meeting notes file. Defaults to sample_notes.txt",
    )
    ...
    print("  Multi-Agent Pipeline  |  Assignment 03A")
    print(f"  Input : {notes_path}")
    print("=" * 62)

    if not os.path.exists(notes_path):
        print(f"\nERROR: {notes_path} not found.")
        sys.exit(1)

    with open(notes_path, "r", encoding="utf-8") as f:
        raw_notes = f.read()

    run_pipeline(raw_notes)
